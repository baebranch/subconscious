...
public sealed class EngineClient : IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("The engine event socket is not connected.");
        }
    ...
    public static string CreateTurnId() => Guid.NewGuid().ToString("N");

    /// <summary>Send a chat message and wait until the frame has been written to the socket.</summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await SendChatAsync(turnId, threadUuid, content, workspaceUuid, modelId, cancellationToken);
        return turnId;
    }

    /// <summary>Send a chat message with a caller-reserved turn ID.</summary>
    ...
    {
        var turnId = CreateTurnId();
        _ = SendChatAsync(turnId, threadUuid, content, workspaceUuid, modelId);
        return turnId;
    }

    public Task CancelChatAsync(string? turnId, CancellationToken cancellationToken = default) =>
        SendFrameAsync("chat.cancel", new { turn_id = turnId }, turnId, cancellationToken);

    public void CancelChat(string? turnId) => _ = CancelChatAsync(turnId);

    /// <summary>Resolves a user-facing approval request and waits for the response frame to be written.</summary>
    public Task ResolveToolApprovalAsync(
        string turnId,
        string approvalId,
        bool approve,
        CancellationToken cancellationToken = default) =>
        SendFrameAsync("tool.approval.response", new
        {
    ...
    public void ResolveToolApproval(string turnId, string approvalId, bool approve) =>
        _ = ResolveToolApprovalAsync(turnId, approvalId, approve);
    ...
    /// <summary>Creates a new Engine-authorized workspace-relative directory.</summary>
    public Task<WorkspaceFileEntry> CreateWorkspaceDirectoryAsync(
        string workspaceUuid, int rootIndex, string relativePath, CancellationToken cancellationToken = default) =>
        SendJsonAsync<WorkspaceFileEntry, CreateWorkspaceDirectoryRequest>(
            HttpMethod.Post,
            $"workspaces/{Uri.EscapeDataString(workspaceUuid)}/files/directory?rootIndex={rootIndex}&path={Uri.EscapeDataString(relativePath)}",
            new CreateWorkspaceDirectoryRequest(),
            cancellationToken);
